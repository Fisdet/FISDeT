            ...
            for row in reader:
                ...
                while i<len(fi.varI):
                        ...
                        if float(row[i])!=0:
                            ...
                            while j<len(fi.termI):
                                if fi.varI[i].getNome()==fi.termI[j].getNomev():
                                    out_file.write(str(round(system.variables[fi.varI[i].getNome()].adjectives[fi.termI[j].getNomet()].getMembership(),2))+';')
                                j=j+1
                        i=i+1
                system.inference()
                z=0
                while z<g.listReg.count():
                    out_file.write(str(round(system.rules['first.'+str(z)].operator.__call__(),2)))#+';')
                    out_file.write(''+';')
                    z=z+1
                out_file.write('\n')
                g.picG_6.setVisible(True)
                g.picR_6.setVisible(False)
                g.msg.setText("Log file storage")
                g.msg.show()
        else:
            out_file.write("\nERROR: invalid CSV")
...
g.testWidget.connect(g.tastoImportTest,QtCore.SIGNAL('clicked()'),importData)
